...
from .mAP import mean_average_precision, mean_average_precision_bits
...
def compute_retrieval_metrics(query, query_class, database, database_class, bits=False):
    query_class = F.one_hot(query_class, num_classes=1000) > 0
    database_class = F.one_hot(database_class, num_classes=1000) > 0
    if bits:
        ap200, recall200, prec200, allIds = mean_average_precision_bits(database, query, database_class, query_class, 200)
    else:
        ap200, recall200, prec200, allIds = mean_average_precision(database, query, database_class, query_class, 200)
    return {'aps@200': float(ap200.mean()), 'prec@200': float(prec200.mean()), 'recall@200': float(recall200.mean()), "allAP": ap200, "allid": allIds}
